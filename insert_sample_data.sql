...
BEGIN
    SELECT id, display_name INTO current_user_id, current_user_name
    FROM sn_users
    WHERE id = auth.uid();

    IF current_user_id IS NULL THEN
        RAISE EXCEPTION '로그인된 사용자가 없습니다. Supabase Dashboard에서 SQL을 실행하면 auth.uid()가 작동하지 않을 수 있습니다.';
    END IF;

    RAISE NOTICE '현재 사용자: % (%)', current_user_name, current_user_id;
END $$;
